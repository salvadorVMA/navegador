...
def handle_auto_next_step(n_intervals, chat_data, session_data):
    ...
    try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    ...
                    try:
                        ...
                        try:
                            ...
                            from __main__ import process_agent_response
                            return process_agent_response(agent_response, session_data, chat_data, user_message)
                        except (ImportError, NameError) as e:
                            print(f"❌ Could not import process_agent_response: {e}")
                            # Fall back to simplified response handling
                            from __main__ import format_chat_history, get_mock_agent_response
                            content = agent_response.get('content', '') 
                            if not content and 'messages' in agent_response:
                                messages = agent_response['messages']
                                if messages and len(messages) > 0:
                                    last_msg = messages[-1]
                                    if isinstance(last_msg, dict):
                            ...
                            # If still no content, use mock
                            if not content:
                                print("⚠️ No content extracted from agent response, using mock")
                                mock_response = get_mock_agent_response(user_message, session_data, search_keywords, preferred_datasets)
                                content = mock_response["content"]
                            ...
                            # Update session data with agent response
                            if isinstance(agent_response, dict):
                                if 'intent' in agent_response:
                                    session_data['intent'] = agent_response['intent']
                                if 'selected_variables' in agent_response:
                                    session_data['variables'] = agent_response['selected_variables']
                                if 'dataset' in agent_response:
                                    session_data['datasets'] = agent_response['dataset']
                        ...
                        if is_health_query:
                            ...
                            try:
                                ...
                                print("⚠️ Could not import get_mock_agent_response")
                                mock_response = {
                                    "content": "I'm having trouble processing your request. Could you try again with a different question?",
                                    "session_updates": {}
                                }
                        ...
                        try:
                            ...
                            return format_chat_history(new_chat_data), new_chat_data, "", session_data, True
                        except (ImportError, NameError) as format_err:
                            print(f"❌ Could not import format_chat_history: {format_err}")
                            # Emergency fallback with minimal formatting
                            new_chat_data = chat_data.copy() if chat_data else []
                            new_chat_data.append({
                                "type": "assistant",
                                "content": mock_response["content"],
                            ...
                            session_data['pending_main_action']['processed'] = True
                            session_data['pending_main_action']['processed_at'] = time.time()
                            # Just return the chat data directly - the UI will handle it
                        ...
                        try:
                            ...
                            return format_chat_history(new_chat_data), new_chat_data, "", session_data, True
                        except (ImportError, NameError) as fallback_err:
                            print(f"❌ Could not import functions for fallback: {fallback_err}")
        ...
        try:
            ...
            from __main__ import get_message, format_chat_history
