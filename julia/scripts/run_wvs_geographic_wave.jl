# run_wvs_geographic_wave.jl — Parameterized per-wave geographic sweep
#
# Adapts run_wvs_geographic_fast.jl to sweep ANY wave (not just W7).
# Accepts wave number as first CLI argument.
#
# The flat-parallel strategy flattens ALL (country, pair) combos for
# the given wave into one thread pool. Checkpoint/resume via JSON.
#
# Run:
#   julia -t 8 --project=. scripts/run_wvs_geographic_wave.jl 6
#   julia -t 8 --project=. scripts/run_wvs_geographic_wave.jl 5
#
# Output:
#   data/results/wvs_geographic_sweep_w{N}.json
...
# ── Wave from CLI ─────────────────────────────────────────────────────────────
if isempty(ARGS)
    println("Usage: julia -t 8 scripts/run_wvs_geographic_wave.jl <WAVE_NUMBER>")
    println("  e.g.: julia -t 8 scripts/run_wvs_geographic_wave.jl 6")
    exit(1)
end
const WAVE = parse(Int, ARGS[1])
@assert 1 <= WAVE <= 7 "Wave must be 1-7, got $WAVE"
...
const OUTPUT_JSON  = joinpath(OUTPUT_DIR, "wvs_geographic_sweep_w$(WAVE).json")
...
println("=" ^ 70)
println("NavegadorBridge — Geographic Sweep Wave $WAVE (flat-parallel)")
println("=" ^ 70)
println("Start: $(now())")
...
wave_prefix = "WVS_W$(WAVE)_"
wave_contexts = sort([k for k in keys(manifest) if startswith(k, wave_prefix)])
pairs_df = CSV.read(PAIRS_CSV, DataFrame)
@printf("Wave %d: %d contexts, %d pair templates\n\n", WAVE, length(wave_contexts), nrow(pairs_df))

if isempty(wave_contexts)
    println("No contexts found for wave $WAVE. Check manifest.")
    println("Available prefixes: $(sort(unique([split(k,'_')[1]*'_'*split(k,'_')[2] for k in keys(manifest)])))")
    exit(1)
end

# ── Load all CSVs into memory ────────────────────────────────────────────────
println("Loading country CSVs for wave $WAVE...")
t_load = time()
context_dfs = Dict{String, DataFrame}()
for ctx_key in wave_contexts
    csv_path = manifest[ctx_key]["csv_path"]
    if isfile(csv_path)
        context_dfs[ctx_key] = CSV.read(csv_path, DataFrame;
                                         missingstring=["", "NA", "NaN", "nan"])
    else
        println("  WARNING: missing CSV for $ctx_key: $csv_path")
    end
end
...
println("Starting flat-parallel sweep for wave $WAVE...")
...
println("\n" * ("=" ^ 70))
@printf("Wave %d sweep complete in %.1fs (%.2f hours)\n", WAVE, elapsed, elapsed/3600)
