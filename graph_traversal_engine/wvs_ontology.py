...
def _shared_driver(fp_a: Fingerprint, fp_b: Fingerprint) -> Optional[str]:
    ...
    Returns the dimension with max |ρ_a × ρ_b| — the dimension where both
...
class ConstructProfile:
    ...
    ses_summary: dict[str, float]   # {dim: ρ} for all 4 dims
...
class PathResult:
    ...
    total_cost: float             # Sum of -log(|γ|) along path
    signal_chain: float           # Product of |γ| along path
    direct_edge: Optional[float]  # γ if direct edge exists
    attenuation_warning: bool     # signal_chain < 0.001
    path_sign: int                # Product of sign(γ) along path
...
class WVSOntologyQuery:
    ...
    def get_profile(self, key: str) -> ConstructProfile:
        ...
        return ConstructProfile(
            ...
            degree=degree,
            hub_rank=hub_rank,
            mediator_rank=None,  # Phase 3: Floyd-Warshall ranking
    ...
    def get_neighbors(
        ...
        """
        Get significant neighbors of a construct with edge metadata.

        Returns EdgeInfo list sorted by |γ| descending.
        """
        ...
        for neighbor_key, gamma in raw:
            ...
            if fp_source and fp_target:
                ...
                v_s = fp_source.to_vec()
                v_t = fp_target.to_vec()
                fp_cos = _cosine_sim(v_s, v_t)

            edges.append(EdgeInfo(
                source=key,
                target=neighbor_key,
                gamma=gamma,
                ci_width=None,  # CI not stored in weight matrix; available in sweep JSON
                dominant_dim=driver,
    ...
    def get_neighborhood(
        ...
        Mirrors opinion_ontology.OntologyQuery.get_neighborhood but context-aware.
        """
    ...
    def get_camp_members(self, camp_id: int) -> list[str]:
        ...
        return [
            k for k, c in self._graph.camps.items()
    ...
    def get_frustrated_nodes(
        ...
        """Return constructs at the bipartition boundary (high frustrated ratio)."""
        return [
    ...
    def find_path(
        ...
        Edge weight = -log(|γ|). Minimizing total weight ≡ maximizing the
        product of |γ| along the path (strongest SES-mediated chain).

        Sign accumulation: track product of sign(γ) along path to determine
        whether the endpoints are expected to co-vary (+1) or counter-vary (-1).
        """
        ...
        # Build adjacency with -log(|γ|) weights
        n = self._n
        # Dijkstra
    ...
    def compare_with(
        self, other_country: str, key: str
    ) -> dict:
        """
        Compare a construct's profile across two countries.

        Returns fingerprint comparison, camp membership, and edge agreement
        for the construct in both contexts.
        """
        other_graph = self._family.get(other_country, self._wave)
        if other_graph is None:
            return {"error": f"No graph for {other_country} W{self._wave}"}
        ...
        # Fingerprint cosine between the same construct in two countries
        fp_cos = 0.0
        if fp_self and fp_other:
            fp_cos = _cosine_sim(fp_self.to_vec(), fp_other.to_vec())

        # Compare significant neighbors
        neighbors_self = set(
        ...
        jaccard = len(shared) / len(neighbors_self | neighbors_other) if (neighbors_self | neighbors_other) else 0.0

        # Sign agreement on shared edges
        sign_agree = 0
        for nk in shared:
        ...
        return {
            ...
            "camp_agreement": (camp_self and camp_other and
                               camp_self.camp_id == camp_other.camp_id),
