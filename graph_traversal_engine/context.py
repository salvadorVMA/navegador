...
class Fingerprint:
    ...
    Each component is Spearman ρ between the construct score and one SES
...
class ContextGraph:
    ...
    weight_matrix: np.ndarray        # n×n signed γ (NaN = missing/non-significant)
    ...
    bp_lift_matrix: Optional[np.ndarray] = None    # n×n KL lift
    ...
    def get_edge(self, key_a: str, key_b: str) -> Optional[float]:
        """Return γ weight between two constructs, or None if no edge."""
    ...
    def get_significant_neighbors(
        ...
        """Return (neighbor_key, γ) pairs for significant edges above threshold."""
...
class GraphFamily:
    ...
    spectral_distances: Optional[np.ndarray] = None       # n_countries × n_countries
