...
## TDA Pipeline (Topological Data Analysis) — 2026-03-27
...
Python (ripser, persim, scikit-learn) handles TDA + visualization. Julia handles O(n³) bottlenecks (Floyd-Warshall, Sinkhorn OT, spectral eigendecomposition, GW alignment). Communication via CSV scratch files.
...
| `scripts/debug/tda_persistent_homology.py` | Phase 4: ripser H0/H1/H2 + bottleneck distances |
| `scripts/debug/tda_embeddings_and_zones.py` | Phase 5: MDS + zone coherence permutation tests |
...
| Script | Purpose |
|--------|---------|
| `tda_floyd_warshall.jl` | Phase 1: APSP + mediator scores (<1s) |
| `tda_ricci_curvature.jl` | Phase 2: Ollivier-Ricci via Sinkhorn OT (~5 min, 8T) |
| `tda_spectral_distances.jl` | Phase 3: 66×66 Laplacian spectral distances (~2s) |
| `tda_gromov_wasserstein.jl` | Phase 6: all 2145-pair GW alignment (~2s) |
| `tda_temporal_spectral.jl` | Temporal Phase 5: per-wave FW + spectral features |

### Key TDA Findings (WVS W7, 66 countries)
...
| Most similar pair | LBY ↔ MAC (GW=0.009) | |
| Most different pair | GBR ↔ MNG (GW=0.104) | |
| Top mediator (MEX/BRA) | gender_role_traditionalism | Varies by country |
| Top mediator (USA/JPN) | science_skepticism | |

### Temporal TDA (Mexico W3-W7, 1996-2018)

| Metric | Value |
|--------|-------|
| Pairs scored | 1,073 (288 in 3+ waves) |
| Sign-stable | 78% |
| Strongest upward | outgroup_trust × social_intolerance (+0.105/dec, R²=0.97) |
| Strongest downward | child_autonomy × outgroup_trust (-0.091/dec, R²=0.82) |
| Fiedler value range | 0.670 (1996) → 0.816 (2007) → 0.726 (2018) |
| Spectral entropy | ~3.928 (constant — network complexity stable) |

### TDA Output Files

All TDA outputs in `data/tda/` (CSVs gitignored, pushed to `navegador_data`):
...
| `alignment/` | gw_distance_matrix.csv, gw_alignment_summary.json, gw_transport_top50.json |
| `temporal/` | temporal_trajectory.json, edge_confidence.csv, gamma_trajectories_v2.png |
